...
def _researcher_token(db, researcher):
    user = researcher_service.get_linked_user(db, researcher.id)
    return {"Authorization": f"Bearer {make_token(user)}"}
...
class TestListMilestones:
    def test_empty(self, client, db):
        ...
        resp = client.get(f"/api/researchers/{r.id}/milestones/", headers=_auth(client, db))
    ...
    def test_returns_created(self, client, db):
        ...
        client.post(f"/api/researchers/{r.id}/milestones/", json=PAYLOAD, headers=headers)
        resp = client.get(f"/api/researchers/{r.id}/milestones/", headers=headers)
        assert len(resp.json()) == 1
        assert resp.json()[0]["title"] == "Artigo A"

    def test_404_unknown_researcher(self, client, db):
        resp = client.get("/api/researchers/9999/milestones/", headers=_auth(client, db))
...
class TestCreateMilestone:
    def test_professor_can_create(self, client, db):
        ...
        resp = client.post(f"/api/researchers/{r.id}/milestones/", json=PAYLOAD, headers=_auth(client, db))
        assert resp.status_code == 201
        assert resp.json()["type"] == "publicacao"
        assert resp.json()["researcher_id"] == r.id
    ...
    def test_own_researcher_can_create(self, client, db):
        ...
        resp = client.post(f"/api/researchers/{r.id}/milestones/", json=PAYLOAD, headers=headers)
        assert resp.status_code == 201
    ...
    def test_other_researcher_cannot_create(self, client, db):
        ...
        resp = client.post(f"/api/researchers/{r2.id}/milestones/", json=PAYLOAD, headers=headers)
        assert resp.status_code == 403

    def test_invalid_type_returns_422(self, client, db):
        r = make_researcher(db, nome="R Invalid", email="r_invalid_ms@u.br")
        resp = client.post(
            f"/api/researchers/{r.id}/milestones/",
...
class TestUpdateMilestone:
    def test_professor_can_update(self, client, db):
        ...
        headers = _auth(client, db)
        created = client.post(f"/api/researchers/{r.id}/milestones/", json=PAYLOAD, headers=headers).json()
        resp = client.put(
            f"/api/researchers/{r.id}/milestones/{created['id']}",
    ...
    def test_404_unknown_milestone(self, client, db):
        ...
        resp = client.put(
            f"/api/researchers/{r.id}/milestones/9999",
            json={"title": "X"},
            headers=_auth(client, db),
        )
        assert resp.status_code == 404
...
class TestDeleteMilestone:
    def test_professor_can_delete(self, client, db):
        ...
        headers = _auth(client, db)
        created = client.post(f"/api/researchers/{r.id}/milestones/", json=PAYLOAD, headers=headers).json()
        resp = client.delete(f"/api/researchers/{r.id}/milestones/{created['id']}", headers=headers)
        assert resp.status_code == 204
        remaining = client.get(f"/api/researchers/{r.id}/milestones/", headers=headers).json()
        assert remaining == []
    ...
    def test_other_researcher_cannot_delete(self, client, db):
        ...
        created = client.post(f"/api/researchers/{r2.id}/milestones/", json=PAYLOAD, headers=prof_headers).json()
        r1_headers = _researcher_token(db, r1)
        resp = client.delete(f"/api/researchers/{r2.id}/milestones/{created['id']}", headers=r1_headers)
        assert resp.status_code == 403
