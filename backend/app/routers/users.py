...
def update_user_profile(
    ...
    """Qualquer usuário edita o próprio perfil; professor/superadmin edita qualquer um."""
    user = user_service.get_by_id(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    if current.role not in ("professor", "admin", "superadmin") and current.id != user_id:
